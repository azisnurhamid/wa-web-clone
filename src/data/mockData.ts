...
const generateAIConversation = (user: User, isRomantic: boolean = false): ChatSession => {
  ...
  for (let i = 0; i < messageCount; i++) {
    ...
    const prefix = isRomantic ? getRandomItem(romanticPrefixes) : getRandomItem(casualPrefixes);
    ...
    messages.push({
      ...
      isMine: isMine,
      status:
        getRandomInt(1, 100) <= 40 ? 'read' : getRandomInt(1, 100) <= 50 ? 'delivered' : 'sent',
    });
  }
...
console.log(
  `Generated ${ALL_CONTACTS.length} contacts and ${CHAT_SESSIONS.length} chat sessions by AI.`,
);
