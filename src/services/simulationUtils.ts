...
export const generateAIResponse = (userMessage: string, isSecretChat: boolean): string => {
  ...
  if (
    ...
    message.includes('enggak') ||
    message.includes('ga') ||
    message.includes('gak')
  ) {
    return getRandomItem(botReplies.responses.no);
  }

  if (
    message.includes('terima') ||
  ...
  if (
    message.includes('siang') ||
    message.includes('sore') ||
    message.includes('pagi') ||
    message.includes('malam')
  ) {
    return getTimeGreeting();
  }
...
const getRandomMessageText = (): string => {
  ...
  switch (category) {
    case 1:
      return getRandomItem(botReplies.conversations.greetings);
    case 2:
      return getRandomItem(botReplies.conversations.questions);
    case 3:
      return getRandomItem(botReplies.conversations.statements);
    case 4:
      return getRandomItem(botReplies.conversations.businessTalk);
    default:
      return 'Oke';
  }
};
