...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  if (searchQuery) {
    displayChats = displayChats.filter(
      (c) =>
  ...
  const handleAction = (
    ...
    switch (action) {
      case 'archive':
        onUpdateChat(chatId, { archived: true });
        break;
      case 'unarchive':
        onUpdateChat(chatId, { archived: false });
        break;
      case 'pin':
        if (pinnedChatsCount < 3) {
          ...
          alert(
            TEXTS.sidebar.maxPinError ||
              'Maksimal 3 chat yang dapat di-pin. Silakan lepas pin chat lain terlebih dahulu.',
          );
        }
        break;
      case 'unpin':
        onUpdateChat(chatId, { pinned: false });
        break;
    }
  };

  const contactsForDrawer =
  ...
  return (
    <div
      className={`flex flex-col w-full md:w-[35%] md:min-w-[320px] md:max-w-[450px] h-full border-r border-gray-300 bg-white relative ${className}`}
    >
      <ProfileDrawer
        isOpen={view === 'PROFILE'}
            ...
            <div className="h-16 bg-[#008069] flex items-end px-4 pb-4 shrink-0 transition-all duration-200 z-30">
              <div className="flex items-center gap-4 text-white w-full">
                <button
                  ...
                  className="hover:bg-white/10 p-2 rounded-full transition mr-2"
                >
                  <ArrowLeft size={24} />
                </button>
          ...
          <>
            ...
            <div className="bg-white px-3 py-2 border-b border-gray-100 flex items-center gap-2">
              <div className="flex-1 flex items-center bg-[#f0f2f5] rounded-lg px-4 py-1.5 focus-within:bg-white focus-within:shadow-sm border border-transparent focus-within:border-white transition-all">
                <div
                  className={`transition-transform duration-300 ${searchQuery ? 'rotate-90 scale-0 w-0' : 'rotate-0 scale-100'}`}
                >
                  <Search size={18} className="text-[#54656f] mr-4" />
                </div>
                {searchQuery && (
                  <button
                    ...
                    className="mr-4 text-[#00a884] animate-in fade-in zoom-in"
                  >
                    <ArrowLeft size={18} />
                  </button>
                )}
                <input
                  ...
                  placeholder={
                    isInteractionLocked
                      ? TEXTS.sidebar.searchDisabled
                      : isUnreadFilter
                        ? TEXTS.sidebar.searchUnread
                        : TEXTS.sidebar.searchPlaceholder
                  }
