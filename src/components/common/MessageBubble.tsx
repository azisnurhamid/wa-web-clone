  ...
  return (
    <div className={`flex w-full mb-2 ${isMine ? 'justify-end' : 'justify-start'}`}>
      <div
        ...
        `}
      >
        <span className="text-gray-900 leading-relaxed block pr-7 break-words">{message.text}</span>

        <div className="flex justify-end items-center gap-1 mt-[-4px] min-w-[50px] float-right">
          <span className="text-[10px] text-gray-500">{message.timestamp}</span>
          {isMine && (
            <span
              className={`text-[14px] ${message.status === 'read' ? 'text-[#53bdeb]' : 'text-gray-400'}`}
            >
