...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({
  ...
  const cursorClass =
    isPrivacyMode && isInteractionLocked ? 'cursor-not-allowed' : 'cursor-pointer';
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="py-7 flex justify-center">
          <div
            className={`relative group ${cursorClass} w-[200px] h-[200px] transition-all duration-300 ${isPrivacyMode ? `blur-[8px] grayscale-[50%] ${!isInteractionLocked ? 'hover:blur-0 hover:grayscale-0' : ''}` : ''}`}
          >
            <DefaultAvatar size={200} />
            {isPrivacyMode ? (
              <div
                className={`absolute inset-0 rounded-full flex flex-col items-center justify-center text-white transition-opacity ${isInteractionLocked ? 'opacity-100 bg-black/40' : 'opacity-0 hover:opacity-100 bg-black/40'}`}
              >
                {isInteractionLocked ? null : (
                  <>
                    <Camera size={24} className="mb-2" />
                    <span className="text-xs uppercase text-center w-24">
                      {TEXTS.profile.changePhoto}
                    </span>
                  </>
                )}
              </div>
        ...
        <div className="bg-white px-8 py-4 shadow-sm mb-3">
          ...
          {isEditingName ? (
              ...
              <div className="flex items-center justify-between border-b-2 border-[#00a884] py-1">
                ...
                <div className="flex items-center gap-3 text-[#8696a0]">
                  ...
                  <button
                    ...
                    className="text-[#00a884] hover:bg-gray-100 p-1 rounded"
                  >
                    <Check size={20} />
                  </button>
                </div>
            ...
            <div
              className={`flex items-center justify-between group ${cursorClass}`}
              onClick={() => {
            ...
            >
              <span
                className={`text-[#3b4a54] text-[17px] transition-all duration-300 ${blurClass}`}
              >
                {name}
              </span>
        ...
        <div className="px-8 py-2 text-[#667781] text-[14px] mb-4">{TEXTS.profile.aboutHint}</div>
