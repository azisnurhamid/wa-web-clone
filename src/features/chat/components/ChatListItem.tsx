    ...
    >
      ...
      >
        {chat.user.statusUpdates && chat.user.statusUpdates.length > 0 && (
          <StatusRing
            count={chat.user.statusUpdates.length}
            hasUnviewed={chat.user.statusUpdates.some((s) => !s.isViewed)}
          />
        )}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        <div className="flex justify-between items-baseline mb-0.5">
            ...
            {searchQuery ? (
              <span
                dangerouslySetInnerHTML={{
                  __html: chat.user.name.replace(
                    new RegExp(`(${searchQuery})`, 'gi'),
                    '<span class="text-[#00a884] font-bold">$1</span>',
                  ),
                }}
              />
            ) : (
          ...
          <span
            className={`text-xs ${chat.unreadCount > 0 ? 'text-[#00a884] font-medium' : 'text-[#667781]'} transition-all duration-300 ${blurClass}`}
          >
        ...
        <div className="flex justify-between items-center group-hover:pr-6 relative">
          <p
            className={`text-[14px] text-[#667781] truncate pr-2 flex-1 transition-all duration-300 ${blurClass}`}
          >
            {chat.isTyping ? (
              <span className="text-[#00a884] italic">{TEXTS.sidebar.typing}</span>
            ) : (
              chat.lastMessage
            )}
          </p>
          <div className="flex items-center gap-2">
            {chat.pinned && (
              <span
                className={`text-[#667781] transition-all duration-300 ${blurClass}`}
                title="Chat di-pin"
              >
            ...
            {chat.unreadCount > 0 && (
              <span
                className={`bg-[#00a884] text-white text-[10px] font-bold h-5 min-w-[20px] px-1 flex items-center justify-center rounded-full shrink-0 transition-all duration-300 ${blurClass}`}
              >
                {chat.unreadCount}
              </span>
            )}
      ...
      {menuOpenId === chat.id && (
        <div
          ref={menuRef}
          className="absolute right-8 top-8 bg-white shadow-xl rounded-md py-2 z-50 w-48 border border-gray-100"
        >
          <ul className="text-[#3b4a54] text-[14.5px]">
            {chat.pinned ? (
              <li
                onClick={(e) => onAction(e, chat.id, 'unpin')}
                className="px-6 py-2 hover:bg-[#f0f2f5] cursor-pointer flex items-center gap-2"
              >
                <PinOff size={16} /> {TEXTS.sidebar.unpinChat}
              </li>
            ) : (
              <li
                onClick={(e) => onAction(e, chat.id, 'pin')}
                className="px-6 py-2 hover:bg-[#f0f2f5] cursor-pointer flex items-center gap-2"
              >
            ...
            {chat.archived ? (
              <li
                onClick={(e) => onAction(e, chat.id, 'unarchive')}
                className="px-6 py-2 hover:bg-[#f0f2f5] cursor-pointer"
              >
                {TEXTS.sidebar.unarchiveChat}
              </li>
            ) : (
              <li
                onClick={(e) => onAction(e, chat.id, 'archive')}
                className="px-6 py-2 hover:bg-[#f0f2f5] cursor-pointer"
              >
                {TEXTS.sidebar.archiveChat}
              </li>
            )}
            <li className="px-6 py-2 hover:bg-[#f0f2f5] cursor-pointer">
              {TEXTS.sidebar.markUnread}
            </li>
            <li className="px-6 py-2 hover:bg-[#f0f2f5] cursor-pointer">
              {TEXTS.sidebar.deleteChat}
            </li>
          </ul>
        </div>
      )}
...
const StatusRing = ({ count, hasUnviewed }: { count: number; hasUnviewed: boolean }) => {
  ...
  if (count === 1) {
    return (
      <svg
        ...
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[56px] h-[56px] z-10 pointer-events-none"
      >
        <circle
          cx={center}
          cy={center}
  ...
  return (
    <svg
      viewBox={`0 0 ${size} ${size}`}
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[56px] h-[56px] z-10 pointer-events-none -rotate-90"
    >
      <circle
        cx={center}
        cy={center}
